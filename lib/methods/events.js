...
function events(query = {}, options, callback) {
  ...
  const { calendarUuid, startDate, endDate } = query;
  if (!calendarUuid) {
    const error = constructError('{{query.calendarUuid}} must be defined in [events] request', 404);
    callback(error);
    return;
  }
  if (!(startDate || endDate)) {
    const error = constructError('{{query.startDate}} or {{query.endDate}} must be defined in [events] request', 404);
    callback(error);
    return;
  }
