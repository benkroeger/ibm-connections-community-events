...
// parsing links by assigning each link object to its 'rel' value
const parseRecurrence = node =>
  parseXMLNode(
    node,
    {
      frequency: 'string(@frequency)',
      interval: 'number(@interval)',
      until: 'string(snx:until)',
      startDate: 'string(snx:startDate)',
      endDate: 'string(snx:endDate)',
      byDay: 'string(snx:byDay)',
      allday: {
        selector: 'number(snx:allday)',
        transform: toBoolean,
      },
...
module.exports = (xmlNode) => {
  ...
  const result = _.assign(textValueSelectors, {
    ...
    recurrence: {
      selector: 'snx:recurrence',
      transform: parseRecurrence,
    },
  });
